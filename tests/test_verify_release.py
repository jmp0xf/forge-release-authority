...
class CandidateFixture:
    ...
    def verify_with(self, materials: verifier._ResolvedMaterials) -> dict[str, Any]:
        with mock.patch.object(verifier, "AUTHORITY_POLICY_PATH", self.policy_path):
    ...
    def resolved_materials(self) -> verifier._ResolvedMaterials:
        return verifier._ResolvedMaterials(
            ...
            authority_release_io_posix=(
                verifier.AUTHORITY_RELEASE_IO_POSIX_PATH.read_bytes()
            ),
...
class VerifyReleaseTests(unittest.TestCase):
    ...
    def test_github_api_reader_rejects_redirects_and_unbounded_responses(self) -> None:
        ...
        with (
            mock.patch.object(verifier, "build_opener") as build_opener,
            self.assertRaisesRegex(
                verifier.VerificationError,
                "invalid GitHub API response limit",
            ),
        ):
            verifier._github_api_get(
                "/repositories/1",
                verifier.MAX_GITHUB_API_RESPONSE_BYTES + 1,
                "fixture",
            )
        build_opener.assert_not_called()

    def test_source_hard_caps_fail_before_any_repository_request(self) -> None:
        policy = json.loads(POLICY_PATH.read_text(encoding="utf-8"))
        self.assertEqual(
            verifier._source_material_limits(policy),
            {
                "Cargo.lock": verifier.MAX_SOURCE_CARGO_LOCK_BYTES,
                "THIRD-PARTY-LICENSES.txt": verifier.MAX_SOURCE_NOTICE_BYTES,
            },
        )
        for key, hard_cap in (
            ("cargoLockBytes", verifier.MAX_SOURCE_CARGO_LOCK_BYTES),
            ("noticeBytes", verifier.MAX_SOURCE_NOTICE_BYTES),
        ):
            with self.subTest(key=key):
                candidate = copy.deepcopy(policy)
                candidate["limits"][key] = hard_cap + 1
                with (
                    mock.patch.object(verifier, "_resolve_repository_files") as resolve,
                    self.assertRaisesRegex(
                        verifier.VerificationError,
                        rf"policy\.limits\.{key} must be between",
                    ),
                ):
                    verifier._resolve_github_materials(
                        candidate,
                        FORGE_COMMIT,
                        AUTHORITY_COMMIT,
                    )
    ...
    def test_authority_runtime_specs_are_exact_and_bounded(self) -> None:
        original = verifier.AUTHORITY_RUNTIME_SPECS
        self.assertEqual(
            verifier._authority_runtime_limits(),
            dict(original),
        )
        cases = {
            "boolean-limit": ((original[0][0], True), *original[1:]),
            "zero-limit": ((original[0][0], 0), *original[1:]),
            "duplicate-path": (original[0], original[0], *original[2:]),
            "casefold-path": (
                ("scripts/release_io.py", original[0][1]),
                ("SCRIPTS/RELEASE_IO.PY", original[1][1]),
                *original[2:],
            ),
            "path-bytes": (("a" * 257, original[0][1]), *original[1:]),
            "path-components": (
                ("a/a/a/a/a/a/a/a/a", original[0][1]),
                *original[1:],
            ),
            "aggregate": (
        ...
        for name, specs in cases.items():
            with (
                ...
                mock.patch.object(verifier, "AUTHORITY_RUNTIME_SPECS", specs),
                self.assertRaises(verifier.VerificationError),
            ):
                verifier._authority_runtime_limits()

    def test_remote_material_resolution_requests_the_exact_runtime_closure(
        self,
    ) -> None:
        policy = json.loads(POLICY_PATH.read_text(encoding="utf-8"))
        calls: list[tuple[dict[str, Any], str, dict[str, int]]] = []
        runtime_bytes = {
            path: f"runtime:{path}\n".encode("ascii")
            for path, _limit in verifier.AUTHORITY_RUNTIME_SPECS
        }

        def resolve(
            repository: dict[str, Any], commit: str, limits: dict[str, int]
        ) -> dict[str, bytes]:
            calls.append((repository, commit, dict(limits)))
            if commit == FORGE_COMMIT:
                return {
                    "Cargo.lock": LOCK_BYTES,
                    "THIRD-PARTY-LICENSES.txt": NOTICE_BYTES,
                }
            self.assertEqual(commit, AUTHORITY_COMMIT)
            return dict(runtime_bytes)
        ...
        with mock.patch.object(
            ...
            materials = verifier._resolve_github_materials(
                policy,
                FORGE_COMMIT,
                AUTHORITY_COMMIT,
            )
        ...
        self.assertEqual(calls[1][2], dict(verifier.AUTHORITY_RUNTIME_SPECS))
        self.assertEqual(
            tuple(calls[1][2]),
            (
        ...
        self.assertEqual(
            materials.authority_policy,
            runtime_bytes["contracts/release-policy.json"],
        )
        self.assertEqual(
            materials.authority_release_io,
            runtime_bytes["scripts/release_io.py"],
        )
        self.assertEqual(
            materials.authority_release_io_posix,
            runtime_bytes["scripts/release_io_posix.py"],
        )
        self.assertEqual(
            materials.authority_verifier,
            runtime_bytes["scripts/verify_release.py"],
        )
    ...
    def test_authority_runtime_materials_match_every_executed_file(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            fixture = self.fixture(directory)
            materials = fixture.resolved_materials()
            for field in (
                "authority_policy",
                "authority_release_io",
                "authority_release_io_posix",
                "authority_verifier",
            ):
                with self.subTest(field=field), fixture.cli_environment():
                    changed = replace(materials, **{field: b"changed\n"})
                    with self.assertRaisesRegex(
                        verifier.VerificationError,
                        "Authority runtime path .* differs from the protected",
                    ):
                        verifier._verify_local_authority_runtime(changed)
    ...
    def test_json_integer_work_is_bounded(self) -> None:
    ...
    def test_json_objects_reject_duplicate_keys(self) -> None:
        with self.assertRaisesRegex(
            verifier.VerificationError, "duplicate key 'value'"
        ):
            verifier._load_json_bytes(b'{"value":1,"value":2}', "duplicate-key fixture")
    ...
    def test_cli_rechecks_both_created_outputs_before_success(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with fixture.cli_environment():
                modules = verifier._load_release_io(fixture.resolved_materials())
            real_write = modules.posix.write_create_only
            created: list[Any] = []

            def tamper_first_during_second_write(output: Any, data: bytes) -> Any:
                result = real_write(output, data)
                if created:
                    created_view = object.__getattribute__(created[0], "_view")
                    os.lseek(created_view.file_fd, 0, os.SEEK_SET)
                    os.write(created_view.file_fd, b"X")
                    os.fsync(created_view.file_fd)
